...
export async function GET(request: NextRequest) {
    ...
    try {
      ...
      if (error) {
        ...
        if (
          ...
          redirect("/signin?error=Authentication failed. Please try again.");
        }

        redirect("/signin?error=Authentication failed");
      }
  ...
  if (token_hash && type) {
    try {
      ...
      if (error) {
        ...
        redirect("/signin?error=Email verification failed");
      }
  ...
  redirect("/signin?error=Invalid authentication request");
}
